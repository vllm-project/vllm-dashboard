...
| Force-merge stats | `/api/force-merges`; no parameters — returns rate windows (7/30/90/182 days), weekly rate and volume series, top force-merged PR authors, and recent force-merged PRs. A force-merge is a PR merged while `buildkite/ci/pr` was red (failure or error) on its head commit |
