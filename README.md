...
The dashboard's public read-only API is documented by a generated OpenAPI
spec, served at https://ci.vllm.ai/openapi.json (viewable with any OpenAPI
client, e.g. `npx @redocly/cli preview-docs public/openapi.json`).

All documented endpoints are unauthenticated GETs returning JSON; errors use
`{ "error": string }`. Authenticated ingestion/operator routes (OTel traces,
GPU reports, queue reprioritization, cron) are intentionally excluded.

The spec is generated by
[`next-openapi-gen`](https://github.com/tazo90/next-openapi-gen) from a small
JSDoc block and a query-param type colocated in each public
`src/app/api/**/route.ts` — do not edit `public/openapi.json` by hand. When
changing a route's params, update its type and regenerate:

```bash
npm run docs:generate
```

CI fails the PR if the committed `public/openapi.json` is stale.
